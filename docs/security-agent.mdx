...
## Introduction to the Security Agent

The `SecurityAgent` (currently in beta) extends the capabilities of the PandasAI library by adding a Security layer to identify if query can generate malicious code.
...
Creating an instance of the `SecurityAgent` is similar to creating an instance of an `Agent`.
