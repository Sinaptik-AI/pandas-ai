...
## Installation
You can install PandaAI using either `pip` or `poetry` (recommended). Python version `3.8+` and `<3.12` is required.
...
You can get your free API key signing up at [pandabi.ai](https://pandabi.ai). 
...
PandaAI allows you to perform conversational data analysis on multiple datasets. To use this feature, you need to load the dataframes using the `load` method.
