...
There are two ways to use the semantic layer, with the create method (working only for csv and sql) and by defininf a yaml schema file (working for all data sources).
...
### For SQL Databases: using the create method
