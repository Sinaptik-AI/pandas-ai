...
from app.utils.memory import prepare_conv_memory, optimize_memory_with_ai
...
class ChatController(BaseController[User]):
    ...
    async def chat(self, user: UserInfo, chat_request: ChatRequest) -> ChatResponse:
        ...
        memory = optimize_memory_with_ai(memory)
        ...
        # Add AI-enhanced memory to the agent
        if memory:
            agent.context.memory = memory

        # AI-driven response generation
        response = agent.chat(chat_request.query, use_ai=True)
        ...
        # AI-powered log analysis and categorization
        log_summary = agent.pipeline.query_exec_tracker.analyze_with_ai(summary)

        log = await self.logs_repository.add_log(
            user.id, "", log_summary, chat_request.query,
            log_summary['success'], log_summary['execution_time']
        )
