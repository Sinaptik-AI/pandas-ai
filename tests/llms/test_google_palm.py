...
class TestOpenAILLM:
    ...
    def test_initiated_with_wrong_parameters(self):
        with pytest.raises(
            ...
            GooglePalm(api_key="test", temperature=-1)
            GooglePalm(api_key="test", temperature=1.1)

        with pytest.raises(
            ValueError, match=re.escape("top_p must be in the range [0.0, 1.0]")
        ):
            GooglePalm(api_key="test", top_p=-1)
            GooglePalm(api_key="test", top_p=1.1)

        with pytest.raises(
            ValueError, match=re.escape("top_k must be in the range [0.0, 1.0]")
        ):
            GooglePalm(api_key="test", top_k=-1)
            GooglePalm(api_key="test", top_k=1.1)

        with pytest.raises(
            ValueError, match=re.escape("max_output_tokens must be greater than zero")
        ):
            GooglePalm(api_key="test", max_output_tokens=0)
